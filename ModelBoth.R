...
for ( val in seq(from = 0, to = 1, by = 0.01)  ){
  ...
  points(values_log$FPR,values_log$TPR,col="light blue",pch=16)    
  ...
  points(values_l$FPR,values_l$TPR,col="gray",pch=16) 
...
for ( val in seq(from = 0, to = 1, by = 0.01)  ){
    ...
    points(values_log1$FPR,values_log1$TPR,col="light blue",pch=16)    
    ...
    points(values_l1$FPR,values_l1$TPR,col="gray",pch=16)  
...
## K-Means for Applicants
y <- model.matrix(~., data=DataTrainNonApp)[,-1]
y.scaled <- scale(y)
five.clusters <- kmeans(y.scaled, 5, nstart=10)
tapply(DataTrainNonApp$default, five.clusters$cluster, table)

### Lets compute the (Full) PCA
valy <- five.clusters$cluster
data.fips <- DataTrainNum$FIPS
pca.y <- prcomp(y, scale=FALSE)
predict.pc <- predict(pca.y)

##################################################
## Interpreting the factors
loadingsy <- pca.y$rotation

### For each factor lets display the top features that 
## Visualization of top 6 variables

v1<-loadings[order(abs(loadingsy[,1]), decreasing=TRUE)[1:ncol(y)],1]
loadingfit <- lapply(1:ncol(y), function(k) ( t(v1[1:k])%*%v1[1:k] - 1/2 )^2)
v1[1:which.min(loadingfit)]
barplot(100*v1[1:6],space=NULL,las=2,xpd=FALSE , xlab="", ylab = bquote("Percentage %"),ylim=c(0,100), main="PCA of the Cluster 1 (Applicants)",col = 3)

v2<-loadings[order(abs(loadingsy[,2]), decreasing=TRUE)[1:ncol(y)],2]
loadingfit <- lapply(1:ncol(y), function(k) ( t(v2[1:k])%*%v2[1:k] - 1/2 )^2)
v2[1:which.min(loadingfit)]
barplot(abs(100*v2[1:6]),space=NULL,las=2,xpd=FALSE , xlab="", ylab = bquote("Percentage %"),ylim=c(0,100), main="PCA of the Cluster 2 (Applicants)",col = 3)

v3<-loadings[order(abs(loadingsy[,3]), decreasing=TRUE)[1:ncol(y)],3]
loadingfit <- lapply(1:ncol(y), function(k) ( t(v3[1:k])%*%v3[1:k] - 1/2 )^2)
v3[1:which.min(loadingfit)]
barplot(abs(100*v3[1:6]),space=NULL,las=2,xpd=FALSE , xlab="", ylab = bquote("Percentage %"),ylim=c(0,100), main="PCA of the Cluster 3 (Applicants)",col = 3)

v4<-loadings[order(abs(loadingsy[,4]), decreasing=TRUE)[1:ncol(y)],4]
loadingfit <- lapply(1:ncol(y), function(k) ( t(v4[1:k])%*%v4[1:k] - 1/2 )^2)
v4[1:which.min(loadingfit)]
barplot(abs(100*v4[1:6]),space=NULL,las=2,xpd=FALSE , xlab="", ylab = bquote("Percentage %"),ylim=c(0,100), main="PCA of the Cluster 4 (Applicants)",col = 3)

v5<-loadings[order(abs(loadingsy[,5]), decreasing=TRUE)[1:ncol(y)],5]
loadingfit <- lapply(1:ncol(y), function(k) ( t(v5[1:k])%*%v5[1:k] - 1/2 )^2)
v5[1:which.min(loadingfit)]
barplot(abs(100*v5[1:6]),space=NULL,las=2,xpd=FALSE , xlab="", ylab = bquote("Percentage %"),ylim=c(0,100), main="PCA of the Cluster 5 (Applicants)",col = 3)
