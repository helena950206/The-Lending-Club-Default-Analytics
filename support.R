scan<-function(mat){
  ...
  for (i in 1:n){
    if (class(mat[,i])!="numeric" | all(unique(mat[,i]) %in% c(0,1)) ){
      result<-rbind(result,cbind(names(mat)[i],nlevels(factor(mat[,i]))))
      dummy<-dummy+as.numeric(nlevels(factor(mat[,i])))
      
    }
    else{
      result2<-rbind(result2,cbind(names(mat)[i],t(round(as.matrix(summary(mat[,i])),0)),sum(mat[,i]==0)))
    }
    
  }
  result<-data.frame(result)
  result2<-data.frame(result2)
  colnames(result)<-c("Variable","Number of levels")
  colnames(result2)<-c("Variable","Min","1st.Quartile","Median","Mean","3rd.Quartile","Max","Number of zeros")
